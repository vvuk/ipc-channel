...
use libc::{c_char, intptr_t, memcmp};
...
impl MessageReader {
    ...
    fn ov_ptr(&self) -> *mut winapi::OVERLAPPED {
        unsafe { self.ov.deref() as *const _ as *mut winapi::OVERLAPPED }
    }
    ...
    fn notify_completion(&mut self, err: u32)
                         -> Result<(),WinError>
    {
        dd2!("[$ {:?}:{:?}] notify_completion", self.iocp, self.handle);

        let nbytes = unsafe { self.ov.InternalHigh as u32 };
        let offset = unsafe { self.ov.Offset };

        assert!(offset == 0);

        // if the remote end closed...
        if err != winapi::ERROR_SUCCESS {
            dd!("[$ {:?}:{:?}] *** notify_completion: need to handle error! {}", self.iocp, self.handle, err);
            return Err(WinError::from_system(err, "notify_completion"));
        }
        ...
        unsafe {
            ...
            if ok == winapi::FALSE {
                ...
                if err == winapi::ERROR_BROKEN_PIPE {
                    dd!("[$ {:?}:{:?}] BROKEN_PIPE, marking closed", self.iocp, self.handle);
                    self.closed = true;
                    return Ok(());
                }
...
fn take_rwlock(mut the_arc: Arc<RwLock<WinHandle>>) -> RwLock<WinHandle> {
    for _ in 0..10 {
        the_arc = match Arc::try_unwrap(the_arc) {
            Ok(rw) => return rw,
            Err(arc) => arc,
        };
    ...
    panic!("Failed to get exclusive ownership of rwlock from Arc");
}
...
impl OsIpcSender {
                ...
                -> Result<(),WinError>
    {
        ...
        let mut shmem_handles: Vec<intptr_t> = vec![];
        let mut shmem_sizes: Vec<u64> = vec![];
